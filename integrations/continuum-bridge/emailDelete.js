...
const CLEANUP_MARKETING_SENDERS = /\b(auction\.com|adc\.auction|realtytrac|foreclosurefortunes|foreclosure|shopifyemail\.com|@t\.shopifyemail|hit-reply@linkedin|mailer\.appfolio|appfolio\.us|mattressfirm|mattress\s+firm|@welcome\.americanexpress|@infos-|notifications@united\.com|auto-reply@usps\.com|@email\.usps\.gov|informeddelivery\.usps|@convo\.zillow\.com|convo\.zillow|update\.strava\.com|mail@update\.strava|gopassport\.com|mcinfo@ups\.com|shipment-tracking@amazon\.com|order-update@amazon\.com|auto-confirm@amazon\.com|return@amazon\.com|@e\.stessa\.com|stessa\.com)\b/i;
...
function formatEmailCleanupPreviewNextSteps({ dateRangeLabel = null, cleanupCount = 0 } = {}) {
  ...
  lines.push(
    ...
    '- Protected mail (banks, OTP/security, never-trash senders including MassHousing and Boston Sailing Center) is not in the list above.',
